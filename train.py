...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, model_type = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.model
    ...
    if pretrained:
        ...
        if model_type == 'yolov5':
            model = Yolov5(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        elif model_type == 'yolo_repvgg':
